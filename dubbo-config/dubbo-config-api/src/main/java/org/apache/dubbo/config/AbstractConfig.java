...
//                            String pre = (prefix != null && prefix.length() > 0 ? prefix + "." : "");
     ...
     * prividerConfig属性默认值 就是xml 中写的赋的值
     */
    public void refresh() {

        try {
            /**
             * getPrefix() 获取的是dubbo.service+你暴露的服务名
             * getId()获取的就是xml 中的servicebean的id
             * getConfiguration 获取配置信息  配置项具体信息 查看具体方法
             */
            ...
            for (Method method : methods) {
                if (ClassHelper.isSetter(method)) {
                    try {
                        /**
                         * 获取所有配置项中的对应属性的最终值
                         * 并执行set 方法method.invoke
                         * extractPropertyName方法
                         * 是获取set 或者@Parameter的属性key
                         * 如果只有set 就获取set 方法后的名字 ；例如 setxxxx 获取的就是xxxx
                         * 如果有@Parameter 就去获取@Parameter的Key
                         *
                         * 最终返回的是@Parameter 的key 否则返回xxxx
                         */
